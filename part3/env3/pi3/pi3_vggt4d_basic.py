...
def process_scene(scene_dir: Path, output_dir: Path, vggt_model, pi3x_model):
    ...
    # Boost recall by dilating raw dynamic masks around moving boundaries.
    print("  [Processing] Applying Morphological Dilation for Recall boost...")
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
    dilated_masks = [cv2.dilate(m, kernel, iterations=1) for m in raw_dyn_masks]
    dyn_masks_tensor = torch.tensor(np.array(dilated_masks)).bool().to(device)
