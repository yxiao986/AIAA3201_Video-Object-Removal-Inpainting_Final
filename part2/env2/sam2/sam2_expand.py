...
def main() -> None:
    parser = argparse.ArgumentParser(description="SAM2 + YOLO prompts with mask dilation post-processing.")
    parser.add_argument("--image_dir", type=Path, required=True)
    parser.add_argument("--output_dir", type=Path, required=True)
    parser.add_argument("--sam2_repo_root", type=Path, default=None)
    ...
    parser.add_argument("--target_classes", type=parse_classes, default=[2], help="Comma-separated COCO class ids.")
    parser.add_argument("--prompt_interval", type=int, default=15)
    parser.add_argument("--mask_threshold", type=float, default=-0.1)
    parser.add_argument("--dilation_kernel", type=int, default=7)
    parser.add_argument("--dilation_iters", type=int, default=1)
    ...
    print(f"Injecting prompts every {args.prompt_interval} frame(s)...")
    ...
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (args.dilation_kernel, args.dilation_kernel))

    print("Propagating masks and applying dilation...")
    for out_frame_idx, out_obj_ids, out_mask_logits in sam2_predictor.propagate_in_video(inference_state):
        ...
        for i, _ in enumerate(out_obj_ids):
            raw_mask = (out_mask_logits[i, 0].cpu().numpy() > args.mask_threshold).astype(np.uint8)
            dilated_mask = cv2.dilate(raw_mask, kernel, iterations=args.dilation_iters)
            frame_mask = cv2.bitwise_or(frame_mask, dilated_mask * 255)
